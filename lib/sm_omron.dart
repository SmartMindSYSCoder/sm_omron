  ...
  Future<ScannedDevice?> scan({required String deviceIdentifier}) async{
    ...
    if (data is List) {
      ...
      if(deviceModel !=null) {
        ...
        dynamic scanData=  await _methodChannel.invokeMethod('scan', deviceModel.toJson());
...
//         }
  ...
  Future<OmronData?> readDevice({required String deviceIdentifier,required ScannedDevice scannedDevice}) async{
  ...
  if (data is List) {
    ...
    if(deviceModel !=null) {
       ...
       print("*********************  before transfer \n${scannedDevice.toJson()}");
        ...
        dynamic data=  await _methodChannel.invokeMethod('autoTransferData', {'localName':scannedDevice.deviceInformation!.localName,'uuid':scannedDevice.uuid});
...
return null;
    ...
    if(d !=null) {
      ...
      for (var element in d ) {
        ...
        if(element is ScannedDevice){
          ...
          var sd = ScannedDevice(uuid: tt['uuid'],
  ...
  removeDevice(ScannedDevice scannedDevice) async{
    ...
    if(oldSavedData.isNotEmpty){
      ...
      oldSavedData.removeWhere((element) => element.modelName! ==scannedDevice.modelName);
  ...
  Future<OmronData?> readWeight(ScannedDevice scannedDevice) async{
     ...
     sd.category=scannedDevice.deviceInformation!.omronDeviceInformationCategoryKey;

    dynamic data=await  _methodChannel.invokeMethod('weight',scannedDevice.toJson());
